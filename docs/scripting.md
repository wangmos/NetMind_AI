...
请求钩子在代理关键路径的四个观察点上，把只读事件信封异步投递给一个长驻的隔离 Python 工作进程。钩子系统**默认关闭**，语义上只做观察、提取与受控持久化，**不改写任何流量字节**；任何故障（Python 缺失、脚本错误、工作进程崩溃）都静默降级为直通，代理采集不受影响。架构决策见 `docs/adr/ADR-008-hook-worker.md`。
