...
不报错，只是安静地没有任何反应。工作区「脚本」页勾选的挂载点（`hook-config.json` 的 `hooks` 字段）是
总开关，决定这个挂载点在这次采集里"存不存在"：**没勾选的挂载点上，观察与拦截两条路径都不会触发**，
即便脚本在该点声明了命中的 `INTERCEPT` 规则也一样。勾选之后，具体转发哪些流量再由
`OBSERVE`/`INTERCEPT` 的规则决定。
...
正在采集时点"保存"，工作台会经 stdin 向 CoreHost 发送一条热重载指令；CoreHost 按工作区当前的脚本文件与 `hook-config.json` 重建钩子引擎并热切换到代理上——**不重启监听、不断开采集会话、不丢失已捕获的事务**，调试一行正则不必再走"停止采集→改脚本→重新开始采集"整套流程。切换瞬间：新引擎已就绪才会接管，旧引擎切下来之后才优雅关停（限时清队 flush），中间没有"新旧都不生效"的空档。热重载同时应用脚本内容变化与挂载点勾选变化，两者都来自同一次 `hook-config.json` 重建，无需分别处理。

静默抓包模式提权后无法重定向 stdin，走独立的文件信号机制，**不支持热重载**——改完脚本仍需停止后重新开始采集。
